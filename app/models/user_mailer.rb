class UserMailer
  ...
  def self.deliver_confirmation(user)
    Mailer.deliver_generic_email({ :user => user,
      ...
      :body => Proc.new { |template| template.auto_format(I18n.t(:email_body, :scope => [ :authentication, :users, :confirm ], :login => user.login,
  ...
  def self.deliver_forgot_password(user)
    Mailer.deliver_generic_email({ :user => user,
      ...
      :body => Proc.new { |template| template.auto_format(I18n.t(:email_body, :scope => [ :authentication, :passwords, :forgot ], :login => user.login,
